...
class Projects_model extends App_Model
{
    ...
    public function get_tasks($id, $where = [], $apply_restrictions = false, $count = false, $callback = null)
    {
        ...
        return hooks()->apply_filters('get_projects_tasks', $tasks, [
            'project_id' => $id,
            'where'      => $where,
            'count'      => $count,
        ]);
    ...
    public function do_milestones_kanban_query($milestone_id, $project_id, $page = 1, $where = [], $count = false)
    {
        ...
        return $this->get_tasks($project_id, $where, true, $count, function () use ($count, $page, $limit) {
            if ($count == false) {
                if ($page > 1) {
    ...
    public function get_staff_notes($project_id, $staff_id = null)
    {
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'project_notes.staff_id');
        $this->db->where('project_id', $project_id);
        if ($staff_id) {
            $this->db->where('staff_id', $staff_id);
        }
        ...
        return $this->db->get('project_notes')->result_array();
    }
    ...
    public function get_staff_note($note_id, $staff_id = null)
    {
        ...
        return $this->db->get('project_notes')->row();
    }
     ...
     * @param mixed      $type
     * @param mixed|null $taskStatus
     * @param mixed      $type_where
     *
     * @return array
     */
